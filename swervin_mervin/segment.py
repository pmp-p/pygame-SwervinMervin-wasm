...
class Segment:
    ...
    def should_ignore(self, segment):
        ...
        return (
            self.top["camera"]["z"] <= s.CAMERA_DEPTH
            or self.top["screen"]["y"] <= segment.top["screen"]["y"]
            or self.bottom["screen"]["y"] >= self.top["screen"]["y"]
        )
    ...
    def render_grass(self, window):
        ...
        pygame.draw.rect(window, col, (0, y, s.DIMENSIONS[0], height), int(height <= 1))
    ...
    def render_road(self, window):
        ...
        if not self.in_tunnel:
            ...
            # Left gutter.
            pygame.draw.line(window, s.COLOURS["gutter"], (bottom["x"] - bottom["w"], y_bottom), (top["x"] - top["w"], y_top))
        ...
        if (self.index / s.RUMBLE_LENGTH) % 2 == 0:
            ...
            for lane in range(s.LANES - 1):
                ...
                points = [(bottom_left, y_bottom), (bottom_right, y_bottom), (top_right, y_top), (top_left, y_top)]
    ...
    def render_tunnel_roof(self, window, highest_y):
        ...
        if not self.tunnel_end:
            pygame.draw.rect(window, s.COLOURS["tunnel"], (0, 0, s.DIMENSIONS[0], s.DIMENSIONS[1] - highest_y))
        else:
            # I am mirroring the roof and road segment heights here.
            # Not sure if this will work in all circumstances (hills, etc).
            pygame.draw.rect(window, s.COLOURS["tunnel"], (0, 0, s.DIMENSIONS[0], self.top["screen"]["y"] - (s.TUNNEL_HEIGHT / 4)))

    def render_left_tunnel(self, window):
        bottom = self.bottom["screen"]
        y_bottom = s.DIMENSIONS[1] - bottom["y"]

        points = [(0, y_bottom), ((bottom["x"] - bottom["w"]), y_bottom), ((bottom["x"] - bottom["w"]), 0), (0, 0)]
    ...
    def __initialize_line(self, y, height):
        return {"world": {"y": y, "z": (height * s.SEGMENT_HEIGHT)}, "camera": {}, "screen": {}}
