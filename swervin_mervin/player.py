...
class Player:
    ...
    def render(self, window, segment):
        ...
        if abs(segment.curve) > s.MINIMUM_CORNER_SMOKE and self.direction != 0 and self.speed > (self.settings["top_speed"] / 1.2):
            sprite += "_smoke"
            self.__run_screech()
    ...
    def accelerate(self):
        ...
        curr_speed = self.speed_boost * (
            self.speed + ((self.settings["top_speed"] / self.settings["acceleration_factor"]) * self.acceleration)
        )
    ...
    def travel(self, track_length, window):
        ...
        ) / 10**6  # td.total_seconds() not implemented in Python 2.6
    ...
    def set_acceleration(self, keys):
        ...
        if self.crashed:
            ...
            if (self.x > 1.0 or self.x < -1.0) and self.speed > (
                self.settings["top_speed"] / self.settings["offroad_top_speed_factor"]
            ):
                a = a * 3
            else:
